...
/**
 * Pad a string with the given character in the right. Alias .pad
 */
newString.rpad = function(length, character)
{
    return character.pad(length, character);
};

/**
 * Pad a string with the given character in the left
 */
newString.lpad = function(length, character)
{
    character = character || ' ';
    if (length - this.length <= 0)
    {
 ...
 */
newString.truncate = function(length)
{
    var str = this || '';
 ...
 */
newString.removeAccents = function(length)
{
    var t = this,
        a = {
            'œ' : 'oe',
